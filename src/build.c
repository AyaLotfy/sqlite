...
void sqlite3FinishCoding(Parse *pParse)
{
  ...
  */
  ...
  assert(!pParse->isMultiWrite || sqlite3VdbeAssertMayAbort(v, pParse->mayAbort));
  if (v)
  {
    ...
    */
    if (db->mallocFailed == 0 && (DbMaskNonZero(pParse->cookieMask) || pParse->pConstExpr))
    {
      int iDb, i;
      ...
      {
        Schema *pSchema;
        if (DbMaskTest(pParse->cookieMask, iDb) == 0)
          continue;
  ...
  ** exists */
  ...
  if (zDatabase)
  {
    ...
    if (p == 0 && sqlite3StrNICmp(zName, "sqlite_", 7) == 0)
    {
      if (i == 1)
      {
        if (sqlite3StrICmp(zName + 7, &ALT_TEMP_SCHEMA_TABLE[7]) == 0 || sqlite3StrICmp(zName + 7, &ALT_SCHEMA_TABLE[7]) == 0 || sqlite3StrICmp(zName + 7, &DFLT_SCHEMA_TABLE[7]) == 0)
        {
...
)
{
  ...
  ** and code in pParse and return NULL. */
  if ((db->mDbFlags & DBFLAG_SchemaKnownOk) == 0 && SQLITE_OK != sqlite3ReadSchema(pParse))
  {
    return 0;
  }
...
static void SQLITE_NOINLINE deleteTable(sqlite3 *db, Table *pTable)
{
  ...
  ** that no lookaside memory is used in this case either. */
  ...
  {
    pNext = pIndex->pNext;
    assert(pIndex->pSchema == pTable->pSchema || (IsVirtual(pTable) && pIndex->idxType != SQLITE_IDXTYPE_APPDEF));
...
)
{
  ...
  {
    assert(db->init.iDb == 0 || db->init.busy || IN_RENAME_OBJECT || (db->mDbFlags & DBFLAG_Vacuum) != 0);
    iDb = db->init.iDb;
    *pUnqual = pName1;
  }
  return iDb;
...
)
{
  ...
  {
    if ((pParse->nested == 0 && 0 == sqlite3StrNICmp(zName, "sqlite_", 7)) || (sqlite3ReadOnlyShadowTables(db) && sqlite3ShadowTableName(db, zName)))
    {
...
i16 sqlite3TableColumnToStorage(Table *pTab, i16 iCol)
{
  ...
  if ((pTab->tabFlags & TF_HasVirtual) == 0 || iCol < 0)
    return iCol;
...
void sqlite3AddReturning(Parse *pParse, ExprList *pList)
{
  ...
  if (sqlite3HashInsert(pHash, RETURNING_TRIGGER_NAME, &pRet->retTrig) == &pRet->retTrig)
  {
    sqlite3OomFault(db);
  }
}
...
)
{
  ...
  if (nTerm == 1 && pCol && sqlite3StrICmp(sqlite3ColumnType(pCol, ""), "INTEGER") == 0 && sortOrder != SQLITE_SO_DESC)
  {
...
)
{
  ...
  Table *pTab = pParse->pNewTable;
  sqlite3 *db = pParse->db;
  if (pTab && !IN_DECLARE_VTAB && !sqlite3BtreeIsReadonly(db->aDb[db->init.iDb].pBt))
  {
...
void sqlite3AddCollateType(Parse *pParse, Token *pToken)
{
  ...
  if ((p = pParse->pNewTable) == 0 || IN_RENAME_OBJECT)
    return;
  i = p->nCol - 1;
  db = pParse->db;
...
static void identPut(char *z, int *pIdx, char *zSignedIdent)
{
  ...
  needQuote = sqlite3Isdigit(zIdent[0]) || sqlite3KeywordCode(zIdent, j) != TK_ID || zIdent[j] != 0 || j == 0;
...
static char *createTableStmt(sqlite3 *db, Table *p)
{
  ...
  for (pCol = p->aCol, i = 0; i < p->nCol; i++, pCol++)
  {
    ...
    assert(pCol->affinity == SQLITE_AFF_BLOB || pCol->affinity == sqlite3AffinityType(zType, 0));
...
static void convertToWithoutRowidTable(Parse *pParse, Table *pTab)
{
  ...
  */
  for (pIdx = pTab->pIndex; pIdx; pIdx = pIdx->pNext)
  {
    ...
    if (resizeIndexObject(db, pIdx, pIdx->nKeyCol + n))
      return;
  ...
  */
  ...
  for (i = 0; i < pTab->nCol; i++)
  {
    if (!hasColumn(pPk->aiColumn, nPk, i) && (pTab->aCol[i].colFlags & COLFLAG_VIRTUAL) == 0)
      nExtra++;
  }
  if (resizeIndexObject(db, pPk, nPk + nExtra))
    return;
  for (i = 0, j = nPk; i < pTab->nCol; i++)
  {
    if (!hasColumn(pPk->aiColumn, j, i) && (pTab->aCol[i].colFlags & COLFLAG_VIRTUAL) == 0)
    {
...
)
{
  ...
  */
  ...
  assert((p->tabFlags & TF_HasPrimaryKey) == 0 || p->iPKey >= 0 || sqlite3PrimaryKeyIndex(p) != 0);
  assert((p->tabFlags & TF_HasPrimaryKey) != 0 || (p->iPKey < 0 && sqlite3PrimaryKeyIndex(p) == 0));
...
int *fun()
{
  ...
  if (child_pid == 0)
  {
    ...
    while (1 > 0)
    {
      ...
      printf("general irst pos %d\n", *pos);
      printf("general pos2 first %d\n", pos2);
      if (*pos == 0)
      {
        pos2 = 0;
      }
      if (pos2 < maxTemp)
      {
        ...
        //printf("value %d\n", value);
        *pos = pos2;
        printf("in child star pos is  %d and less than\n", *pos);
        printf("in child pos2 is %d and less than 2\n", pos2);
  ...
  else
  {
    ...
    do
    {
      ...
      {
        printf("in parent  %d", tmp[i]);
      }
      printf("in parent pos %d\n", *pos);
      printf("\n");
      pid_t child_pid2 = fork();
      if (child_pid2 == 0)
      {
        printf("parent will return now\n");
        return tmp;
      }
      else
      {
...
)
{
  ...
  int returnCode;
  // char *zName = pSelect->pSrc->a[0].zName;

  // //printf("Name: %s \n", zName);
  // char *s1 = "SELECTC * from ";
  // char *result = malloc(strlen(s1) + strlen(zName) + 1); // +1 for the null-terminator
  ...
  for (i = 0; i < 2; i++)
  {
    ...
    // in real code you would check for errors in malloc here
    strcpy(finalQuery, "INSERTC into t15 valuesc(");

    char *num;
    char buffer2[100];

    if (asprintf(&num, "%d", *(p + i)) == -1)
    {
      perror("asprintf");
    }
    else
  ...
  //sqlite3NestedParse(pParse, "INSERTC into t15 valuesc(500);");
  sqlite3NestedParse(pParse, "SELECTC * FROM t15;");
  //  listenToPort();
  ...
  // //printf("Contents of select statement are %u, %u, %s\n", pSelect->op, pSelect->selId, zName);
  // for (size_t i = 0; i < 1; i++)
  // {
  //   sqlite3NestedParse(pParse, result);
  ...
  // printf("Rest contents of select statement \n");

  // printf(" %u, %c, %u\n", pSelect->pWhere->op, pSelect->pWhere->affExpr, pSelect->pWhere->op2);
}
  ...
  if (pSel)
  {
    ...
    else if (pTable->pCheck)
    {
      ...
      */
      ...
      if (db->mallocFailed == 0 && pParse->nErr == 0 && pTable->nCol == pSel->pEList->nExpr)
      {
...
{
#ifndef SQLITE_OMIT_VIRTUALTABLE
  if ((db->flags & SQLITE_Defensive) != 0 && db->pVtabCtx == 0 && db->nVdbeExec == 0)
  {
    return 1;
  }
#endif
...
static int tableMayNotBeDropped(sqlite3 *db, Table *pTab)
{
  ...
  {
    if (sqlite3StrNICmp(pTab->zName + 7, "stat", 4) == 0)
      return 0;
    if (sqlite3StrNICmp(pTab->zName + 7, "parameters", 10) == 0)
      return 0;
    return 1;
  }
...
static void sqlite3RefillIndex(Parse *pParse, Index *pIndex, int memRootPage)
{
  ...
  iSorter = pParse->nTab++;
  sqlite3VdbeAddOp4(v, OP_SorterOpen, iSorter, 0, pIndex->nKeyCol, (char *)sqlite3KeyInfoRef(pKey), P4_KEYINFO);
...
)
{
  ...
  */
  ...
  assert(pTab != 0);
  assert(pParse->nErr == 0);
  if (sqlite3StrNICmp(pTab->zName, "sqlite_", 7) == 0 && db->init.busy == 0 && pTblName != 0
#if SQLITE_USER_AUTHENTICATION
  ...
  ** it as a covering index */
  assert(HasRowid(pTab) || pTab->iPKey < 0 || sqlite3TableColumnToIndex(pIndex, pTab->iPKey) >= 0);
...
exit_create_index:
  ...
  if (pTab)
  {
    ...
    ** comes after a REPLACE index on the list. */
    ...
    {
      assert(pThis->onError != OE_Replace || pThis->pNext == 0 || pThis->pNext->onError == OE_Replace);
    }
#endif
  }
...
)
{
  ...
  if ((u32)pSrc->nSrc + nExtra > pSrc->nAlloc)
  {
    ...
    if (nAlloc > SQLITE_MAX_SRCLIST)
      nAlloc = SQLITE_MAX_SRCLIST;
