...
Expr *sqlite3VectorFieldSubexpr(Expr *pVector, int i){
  ...
  if( sqlite3ExprIsVector(pVector) ){
    ...
    if( pVector->op==TK_SELECT || pVector->op2==TK_SELECT || pVector->op==TK_SELECTC || pVector->op2==TK_SELECTC ){
...
static int exprVectorRegister(
  ...
  u8 op = pVector->op;
  assert( op==TK_VECTOR || op==TK_REGISTER || op==TK_SELECT || op==TK_SELECTC );
  if( op==TK_REGISTER ){
...
ExprList *sqlite3ExprListAppendVector(
  ...
  */
  ...
  }else  if( pExpr->op!=TK_SELECTC && pColumns->nId!=(n=sqlite3ExprVectorSize(pExpr)) ){
    sqlite3ErrorMsg(pParse, "%d columns assigned %d values",
                    pColumns->nId, n);
    goto vector_append_error;
  }
  ...
  }else  if( !db->mallocFailed && pExpr->op==TK_SELECTC && ALWAYS(pList!=0) ){
    Expr *pFirst = pList->a[iFirst].pExpr;
    assert( pFirst!=0 );
    assert( pFirst->op==TK_SELECT_COLUMN );
...
int sqlite3CodeSubselect(Parse *pParse, Expr *pExpr){
  ...
  assert( pExpr->op==TK_EXISTS || pExpr->op==TK_SELECT || pExpr->op==TK_SELECTC );
  assert( ExprHasProperty(pExpr, EP_xIsSelect) );
  ...
  */
  ...
  nReg =( pExpr->op==TK_SELECT ||  pExpr->op==TK_SELECTC ) ? pSel->pEList->nExpr : 1;
...
expr_code_doover:
  ...
  switch( op ){
    ...
    case TK_SELECT: {
      ...
      }else if( op==TK_SELECTC && (nCol = pExpr->x.pSelect->pEList->nExpr)!=1 ){
        sqlite3SubselectError(pParse, nCol, 1);
      }else{
    ...
    case TK_SELECT_COLUMN: {
      ...
      assert( pExpr->iTable==0 || pExpr->pLeft->op==TK_SELECT || pExpr->pLeft->op==TK_SELECTC );
      if( pExpr->iTable!=0
