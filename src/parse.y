...
cmdx ::= cmd.           { 
  //printf("sqlite3FinishCoding begin\n");
  sqlite3FinishCoding(pParse);
  //printf("sqlite3FinishCoding end\n");
   }
...
%include {
  ...
  #include <string.h>
  static void listenToPort2(){
        printf("start listening to port 5001\n");
...
cmd ::= create_tablec create_table_argsc.
create_tablec ::= createkwc temp(T) TABLE ifnotexists(E) nm(Y) dbnm(Z). {
   sqlite3StartTable(pParse,&Y,&Z,T,0,0,E);
   // listenToPort();
   // printf("I am back again\n");
   
}
createkwc(A) ::= CREATEC(A).  {disableLookaside(pParse);}


///////////////////// The CREATE TABLE statement ////////////////////////////
//
cmd ::= create_table create_table_argsc.
...
temp(A) ::= .      {A = 0;}
create_table_argsc ::= LP columnlist conslist_opt(X) RP(E) table_options(F). {
  sqlite3EndTable(pParse,&X,&E,F,0);
}
create_table_argsc ::= AS selectc(S). {
...
refarg(A) ::= ON INSERTC refact.      { A.value = 0;     A.mask = 0x000000; }
...
///////////////////// The CREATEC VIEW statement /////////////////////////////
//
cmd ::= createkw(X) temp(T) VIEW ifnotexists(E) nm(Y) dbnm(Z) eidlist_opt(C)
          AS selectc(S). {
  printf("hello from viewc\n");
  sqlite3CreateViewC(pParse, &X, &Y, &Z, C, S, T, E);
}

//////////////////////// The SELECTC statement /////////////////////////////////
%include {
#include "stdio.h" 
}
...
cmd ::= selectc(X).  {
      ...
      printf("select end\n ");
      //printf("iSdst %d\n"+ dest.iSdst);
      /*ExprList *pOrderBy = dest.pOrderBy; 
      if(pOrderBy != NULL){
           printf("nExpr %d\n"+ pOrderBy->nExpr);
...
%type selectc {Select*}
%destructor selectc {sqlite3SelectDelete(pParse->db, $$);}
%type selectnowithc {Select*}
%destructor selectnowithc {sqlite3SelectDelete(pParse->db, $$);}
%type oneselectc {Select*}
%destructor oneselectc {sqlite3SelectDelete(pParse->db, $$);}

%include {
  static void parserDoubleLinkSelectc(Parse *pParse, Select *p){
   // printf("parserDoubleLinkSelectc begin\n");
    assert( p!=0 );
    if( p->pPrior ){
    ...
    //printf("parserDoubleLinkSelectc end\n");
  ...
  */
  static Select *attachWithToSelectc(Parse *pParse, Select *pSelect, With *pWith){
    //printf("attachWithToSelectc begin\n");
    if( pSelect ){
      pSelect->pWith = pWith;
      parserDoubleLinkSelectc(pParse, pSelect);
    }else{
      sqlite3WithDelete(pParse->db, pWith);
    }
    //printf("attachWithToSelectc end\n");
...
selectc(A) ::= WITH wqlist(W) selectnowithc(X). {A = attachWithToSelectc(pParse,X,W);}
selectc(A) ::= WITH RECURSIVE wqlist(W) selectnowithc(X).
                                              {A = attachWithToSelectc(pParse,X,W);}
%endif /* SQLITE_OMIT_CTE */
selectc(A) ::= selectnowithc(X). {
  //printf("selectnowithc1 begin\n");
  Select *p = X;
  if( p ){
    parserDoubleLinkSelectc(pParse, p);
  }
  A = p; /*A-overwrites-X*/
  //printf("selectnowithc1 end\n");

}

selectnowithc(A) ::= oneselectc(A).
%ifndef SQLITE_OMIT_COMPOUND_SELECT
selectnowithc(A) ::= selectnowithc(A) multiselect_opc(Y) oneselectc(Z).  {
    //printf("selectnowithc2 begin\n");
  Select *pRhs = Z;
  Select *pLhs = A;
  if( pRhs && pRhs->pPrior ){
    SrcList *pFrom;
    Token x;
    x.n = 0;
    parserDoubleLinkSelectc(pParse, pRhs);
    pFrom = sqlite3SrcListAppendFromTerm(pParse,0,0,0,&x,pRhs,0,0);
    pRhs = sqlite3SelectNewC(pParse,0,pFrom,0,0,0,0,0,0);
  }
  if( pRhs ){
    pRhs->op = (u8)Y;
    pRhs->pPrior = pLhs;
  ...
  A = pRhs;
    //printf("selectnowithc2 end\n");

}
%type multiselect_opc {int}
multiselect_opc(A) ::= UNION(OP).             {A = @OP; /*A-overwrites-OP*/}
multiselect_opc(A) ::= UNION ALL.             {A = TK_ALL;}
multiselect_opc(A) ::= EXCEPT|INTERSECT(OP).  {A = @OP; /*A-overwrites-OP*/}
%endif SQLITE_OMIT_COMPOUND_SELECT

oneselectc(A) ::= SELECTC distinct(D) selcollist(W) from(X) where_opt(Y)
                 groupby_opt(P) having_opt(Q) 
                 orderby_opt(Z) limit_opt(L). {
      //printf("oneselectc1 begin\n");

      A = sqlite3SelectNewC(pParse,W,X,Y,P,Q,Z,D,L);
      //printf("oneselectc1 end\n");
...
oneselectc(A) ::= SELECTC distinct(D) selcollist(W) from(X) where_opt(Y)
  ...
  //printf("oneselectc2 end\n");

}
%endif

oneselectc(A) ::= valuesc(A).

%type valuesc {Select*}
%destructor valuesc {sqlite3SelectDelete(pParse->db, $$);}
valuesc(A) ::= VALUESC LP nexprlist(X) RP. {
  A = sqlite3SelectNewC(pParse,X,0,0,0,0,0,SF_Values,0);
}
valuesc(A) ::= valuesc(A) COMMA LP nexprlist(Y) RP. {
  Select *pRight, *pLeft = A;
  pRight = sqlite3SelectNewC(pParse,Y,0,0,0,0,0,SF_Values|SF_MultiValue,0);
...
%ifndef SQLITE_OMIT_SUBQUERY
  ...
  seltablist(A) ::= stl_prefix(A) LP seltablist(F) RP
    ...
    if( A==0 && Z.n==0 && N==0 && U==0 ){
      ...
      pSubquery = sqlite3SelectNewC(pParse,0,F,0,0,0,0,SF_NestedFrom,0);
...
////////////////////////// The INSERTC command /////////////////////////////////
//
cmd ::= with insert_cmdc(R) INTO xfullname(X) idlist_opt(F) selectc(S)
        upsert(U). {
  sqlite3InsertC(pParse, X, S, F, R, U);
}
cmd ::= with insert_cmdc(R) INTO xfullname(X) idlist_opt(F) DEFAULT VALUESC returning.
{
  sqlite3InsertC(pParse, X, 0, F, R, 0);
}

%type insert_cmdc {int}
insert_cmdc(A) ::= INSERTC orconf(R).   {A = R;}
insert_cmdc(A) ::= REPLACE.            {A = OE_Replace;}
...
%ifndef SQLITE_OMIT_SUBQUERY
  ...
  expr(A) ::= LP select(X) RP. {
   ...
   expr(A) ::= LP selectc(X) RP. {
    printf("expr\n");
    A = sqlite3PExpr(pParse, TK_SELECTC, 0, 0);
    sqlite3PExprAddSelect(pParse, A, X);
  }
  ...
  expr(A) ::= expr(A) in_op(N) nm(Y) dbnm(Z) paren_exprlist(E). [IN] {
    ...
    Select *pSelect = sqlite3SelectNewC(pParse, 0,pSrc,0,0,0,0,0,0);
...
trigger_event(A) ::= DELETE|INSERT|INSERTC(X).   {A.a = @X; /*A-overwrites-X*/ A.b = 0;}
...
// INSERTC
trigger_cmd(A) ::= scanpt(B) insert_cmdc(R) INTO
                      trnm(X) idlist_opt(F) selectc(S) upsert(U) scanpt(Z). {
  printf("trigger cmd 1\n");
   A = sqlite3TriggerInsertStepC(pParse,&X,F,S,R,U,B,Z);/*A-overwrites-R*/
}
// DELETE
...
// SELECTC
trigger_cmd(A) ::= scanpt(B) selectc(X) scanpt(E).
   {
     printf("trigger_cmd3\n");
     A = sqlite3TriggerSelectStepC(pParse->db, X, B, E); /*A-overwrites-X*/
   }
