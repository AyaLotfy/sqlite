...
Select *sqlite3SelectNew(
  ...
  Expr *pLimit          /* LIMIT value.  NULL means not used */
      ...
      //sqlite3DebugPrintf("in SelectNew hello\n");
  ...
  //  sqlite3DebugPrintf("in SelectNew return\n");
...
Select *sqlite3SelectNewC(
  ...
  //sqlite3DebugPrintf("in SelectNewC hello\n");
  Select *pNew, *pAllocated;
  Select standin;
  pAllocated = pNew = sqlite3DbMallocRawNN(pParse->db, sizeof(*pNew) );
  if( pNew==0 ){
 ...
 // sqlite3DebugPrintf("in SelectNewC return\n");
...
//  printf("sqlite3SelectDelete begin\n");
  if( OK_IF_ALWAYS_TRUE(p) ) clearSelect(db, p, 1);
  //printf("sqlite3SelectDelete end\n");
...
static int multiSelectValues(
  ...
  do{
    ...
    assert( p->op==TK_ALL || (p->op==TK_SELECT && p->pPrior==0) || (p->op==TK_SELECTC && p->pPrior==0) );
...
static int convertCompoundSelectToSubquery(Walker *pWalker, Select *p){
  ...
  for(pX=p; pX && (pX->op==TK_ALL || pX->op==TK_SELECT || pX->op==TK_SELECTC); pX=pX->pPrior){}
...
static int countOfViewOptimization(Parse *pParse, Select *p){
  ...
  while( pSub ){
    ...
    //pTerm = sqlite3PExpr(pParse, TK_SELECTC, 0, 0);
