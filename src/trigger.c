...
void sqlite3BeginTrigger(
  ...
  assert( pName2!=0 );
  assert( op==TK_INSERT || op==TK_INSERTC || op==TK_UPDATE || op==TK_DELETE );
  assert( op>0 && op<0xff );
  if( isTemp ){
...
TriggerStep *sqlite3TriggerSelectStepC(
  sqlite3 *db,                /* Database connection */
  Select *pSelect,            /* The SELECT statement */
  const char *zStart,         /* Start of SQL text */
  const char *zEnd            /* End of SQL text */
...
TriggerStep *sqlite3TriggerInsertStepC(
  Parse *pParse,      /* Parser */
  Token *pTableName,  /* Name of the table into which we insert */
  IdList *pColumn,    /* List of columns in pTableName to insert into */
  Select *pSelect,    /* A SELECT statement that supplies values */
  ...
  pTriggerStep = triggerStepAllocate(pParse, TK_INSERTC, pTableName,zStart,zEnd);
  if( pTriggerStep ){
    if( IN_RENAME_OBJECT ){
      pTriggerStep->pSelect = pSelect;
      pSelect = 0;
    ...
    do{
      if( p->op==op && checkColumnOverlap(p->pColumns, pChanges) ){
        ...
        ** us what time of trigger it should be. */
        ...
        mask |= p->tr_tm;
      }else if( p->bReturning && p->op==TK_INSERTC && op==TK_UPDATE
                && sqlite3IsToplevel(pParse) ){
        /* Also fire a RETURNING trigger for an UPSERT */
        mask |= p->tr_tm;
      }
...
void sqlite3CodeRowTrigger(
  ...
  assert( op==TK_UPDATE || op==TK_INSERT || op==TK_INSERTC || op==TK_DELETE );
  ...
  for(p=pTrigger; p; p=p->pNext){
    ...
    */
    ...
    }else if( (p->op==op || (p->bReturning && p->op==TK_INSERTC && op==TK_UPDATE))
     && p->tr_tm==tr_tm 
     && checkColumnOverlap(p->pColumns, pChanges)
    ){
      if( !p->bReturning ){
